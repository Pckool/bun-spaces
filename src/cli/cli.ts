...
import { getRequiredBunVersion, validateCurrentBunVersion } from '../internal/bun-version';
import { logger } from '../internal/logger';
import { initializeWithGlobalOptions } from './global-options';
import { OUTPUT_CONFIG } from './output';
import { defineProjectCommands } from './project-commands';
...
export const createCliProgram = ({
  ...
  const run = async ({ argv = process.argv }: RunCliOptions = {}) => {
    ...
    try {
      const program = new Command('bunx bun-spaces')
