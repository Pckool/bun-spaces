...
import { type Workspace, findWorkspacesFromPackage } from '../workspaces';
import { ERRORS } from './errors';
import { type CreateScriptCommandOptions, type ScriptCommand, createScriptCommand } from './script-command';
...
class _Project implements Project {
  ...
  findWorkspacesByPattern(workspacePattern: string): Workspace[] {
    ...
    return this.workspaces.filter(
      (workspace) =>
        fs.existsSync(workspace.path) && fs.statSync(workspace.path).isDirectory() && regex.test(workspace.name)
    );
  }
  ...
  createScriptCommand(options: CreateProjectScriptCommandOptions): CreateProjectScriptCommandResult {
    ...
    if (!workspace) {
      throw new ERRORS.ProjectWorkspaceNotFound(`Workspace not found: ${JSON.stringify(options.workspaceName)}`);
    }
...
export const createProject = (options: CreateProjectOptions): Project => new _Project(options);
